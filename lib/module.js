...
module.exports = function (moduleOptions) {
  ...
  this.nuxt.hook('generate:done', async () => {
    ...
    let robots = await getRobots.call(this, options)
  ...
  this.addServerMiddleware({
    ...
    async handler (req, res, next) {
      try {
        const robots = await cache.robots.get('robots')
        if (robots === null) {
        ...
        const rules = await getRules.call(moduleContainer, robots, req)
...
function render (rules) {
  return rules.filter(rule => rule.value).map(rule => `${rule.key}: ${String(rule.value).trim()}`).join('\n')
}
